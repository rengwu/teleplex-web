...
export function HypnoferromagnetismCanvas({ ...props }) {
  return (
    <Canvas resize={{ scroll: false }} {...props}>
      <HypnoferromagnetismScene />
    </Canvas>
  );
}

export function HypnoferromagnetismScene() {
  const material = new ShaderToyMaterial(shadercode);
  const geometry = useMemo(() => new PlaneGeometry(), []);

  return (
    <mesh scale={[30, 15, 1]} material={material} geometry={geometry}></mesh>
  );
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	worldtime = ss1(iTime+6.)*0.5 * 0.6;
	float camtime = ss2(iTime+M_PI+6.) * 0.1;
	centerhue = abs(fract(camtime*0.03)-0.5)*2.2+0.4;
